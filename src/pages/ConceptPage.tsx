...
export function ConceptPage({ locale, entityId }: { locale: Locale; entityId: string }) {
  ...
  return <article className="page detail-page"><a className="back-link" href={`/${locale}/concepts`}><ArrowLeft aria-hidden="true" />{locale === "tr" ? "Kavramlara dön" : "Back to concepts"}</a><header className="page-header"><span className="eyebrow">CONCEPT / {entityId.toUpperCase()}</span><h1>{item.title}</h1><p>{item.description ?? item.summary}</p></header>
    <section><h2>{locale === "tr" ? "Neden önemli?" : "Why it matters"}</h2><p>{item.significance ?? item.summary}</p></section>
    <section><h2>{locale === "tr" ? "İlişkiler" : "Relationships"}</h2><ul className="relation-list">{relations.map((relation) => <li key={relation.id}><code>{relation.kind}</code><span>{catalog.locales[locale].entities[relation.sourceId]?.title} → {catalog.locales[locale].entities[relation.targetId]?.title}</span></li>)}</ul></section>
