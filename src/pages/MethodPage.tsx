...
const StatusGrid = ({ locale }: { locale: Locale }) => <div className="method-grid">
  {(locale === "tr" ? [
    ...
    ["Bildirildi", "Kuruluşun yeterli yayımlanmış yöntem olmadan belirttiği yetenek veya sonuç."],
    ["Çıkarım", "Alıntılanan kanıttan sınırlı editör çıkarımı; gerekçe zorunludur."],
...
export function MethodPage({ locale }: { locale: Locale }) {
  return <article className="page method-page"><header className="page-header"><span className="eyebrow">{locale === "tr" ? "KANIT SÖZLEŞMESİ" : "EVIDENCE CONTRACT"}</span><h1>{locale === "tr" ? "Yöntem ve kaynak politikası" : "Method and source policy"}</h1><p>{locale === "tr" ? "Atlas, kaynak keşfi ile yayın kararını birbirinden ayırır. Otomatik tarama yalnız inceleme adayı üretir; hiçbir iddia insan onayı olmadan yayımlanmaz." : "The atlas separates source discovery from publication. Automated scanning only creates review candidates; no claim is published without human approval."}</p></header>
    ...
    <section><h2>{locale === "tr" ? "Doğrulama durumu" : "Verification state"}</h2><p>{locale === "tr" ? "Güncel, bayat, inceleme gerekli ve geri çekildi durumları kaynağın ne zaman ve nasıl kontrol edildiğini gösterir; iddianın kanıt gücünden ayrıdır." : "Current, stale, needs review, and withdrawn describe when and how a source was checked; they are separate from evidentiary strength."}</p></section>
    ...
    <section><h2>{locale === "tr" ? "Güncellik" : "Freshness"}</h2><p>{locale === "tr" ? "Son katalog doğrulaması: 1 Eylül 2026. Kaynak hatası son onaylı içeriği korur; değişen kaynaklar sessizce yayımlanmak yerine inceleme kuyruğuna girer." : "Last catalog verification: September 1, 2026. Source failure preserves the last approved content; changed sources enter review instead of publishing silently."}</p></section>
  </article>;
}
