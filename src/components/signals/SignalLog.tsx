  ...
  return <div className={compact ? "signal-log signal-log--compact" : "signal-log"}>
    {signals.map((signal) => {
      ...
      return <article key={signal.id} data-testid="signal">
        <div className="signal-date"><time dateTime={signal.eventDate ?? undefined}>{formatDate(signal.eventDate, locale)}</time><span>{source.publisher}</span></div>
        ...
        <div className="signal-evidence"><EvidenceToken locale={locale} kind="evidence" value={evidence.evidenceStatus} /><EvidenceToken locale={locale} kind="verification" value={evidence.verificationState} /><a href={source.url} target="_blank" rel="noreferrer">{locale === "tr" ? "Kaynak" : "Source"}<ExternalLink aria-hidden="true" /></a></div>
      </article>;
    })}
  </div>;
}
